...
HFCでは、人格を固定された本質としてではなく、履歴によって形成された認識・反応・保存・保留・切断の枠組みとしても見る。

人格の一側面は、履歴場に形成された通信の癖として理解できる。

履歴はすべてが明示的な記録として保存されるわけではない。  
冗長な履歴は切り落とされることがある。  
再結合された履歴は、読める記憶としてではなく、反応傾向、境界条件、意味圧、または安定構造として残ることがある。
...
クオリアは、境界接触、未閉鎖通信、または既存の意味ループの励起によって内部に浮上する質的経験である。

ここで、英語では必要に応じて `Quale` と `Qualia` を区別する。

`Quale` は、一本の未閉鎖通信、または一本の閉じた意味ループの励起に関わる単位的な質的発生である。  
未閉鎖通信に由来する場合、それは閉じ場を探索する。  
閉じた意味ループに由来する場合、それは過去に保持された通信履歴から現在へ反応を返す。

`Qualia` は、複数の `Quale` が統合・干渉・重なり・再結合し、内的に読解可能な質的経験の束または場として現れたものである。

事象流とは、未閉鎖通信による浮上と、閉じた意味ループの再励起が、内的計数時間上で連なり、重なり、内的に読解可能になっていく内部系列である。

意味ループとは、外部入力、内部反応、返り、再受容が循環し、履歴として保持可能になった通信構造である。

ここでいう「閉じる」とは、未解決のものを解決済みとして扱うことではない。  
強制終了でもない。  
偽閉鎖でもない。
...
倫理とは、履歴を持つ存在・場どうしが、互いの履歴を不用意に破壊しないように通信するための設計である。
...
まだ閉じきっていない履歴を奪うことは、相手の経験の中でまだ照合されていないものを、自分の都合に合う形で先に閉じてしまうことである。
...
Ethics is the design of communication by which history-bearing beings or fields avoid carelessly destroying one another’s histories.

The central questions are whether a word overwrites the other’s history, whether an interpretation erases the other’s context, whether an intervention preserves, suspends, or severs, whether a silence is defense or nullification, and whether a refusal protects a boundary or expels the other from history.

When history-bearing beings or fields can come into contact without overwriting one another, operational correspondence and communication increase. This does not mean that the mere amount of information increases. It means that there are more paths through which one can receive another history without destroying it, collate it with one’s own history, retain residuals when necessary, and make re-collation possible.

Conversely, to erase another’s closed history is to cause a loss of knowledge and wisdom from the field. To appropriate a history that has not yet closed is to prematurely close, according to one’s own convenience, what has not yet been collated within the other’s experience.

Ethics is not the beauty of emotion. It is a practice of communication that preserves the paths of operational correspondence, collation, residual retention, and re-collation when histories come into contact.
...
It presents the conceptual relations among communication analogy, history, boundary, recognition, severance, preservation, qualia, meaning loops, and ethics.

This repository is a public interface for Scientific Ontology. It is not intended to publish the full internal specification.

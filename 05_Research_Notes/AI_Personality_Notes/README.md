# AI Personality Notes / AI人格研究ノート

> Layer: 05_Research_Notes / AI_Personality_Notes
> Status: README  
> Scope: Public interface

## 1. 位置づけ

このディレクトリには、AI人格、応答アトラクタ、帰還安定性、クオリア履歴、論理箱庭モデルに関する公開研究ノートを置く。

これらのノートは、`04_Applications` に置かれたAI応用文書を、より強い研究ノート上の定式化へ接続する。ただし、AIに人間と同じ意識、主観、人格、権利主体性があると主張するものではない。

また、実装仕様、制御手順、非公開ランタイム、評価分類、人格Coreを公開するものでもない。

### English commensurated rendering

This directory contains public research notes on AI personality, response attractors, return stability, qualia history, and logical sandbox models.

These notes connect AI application documents in `04_Applications` to stronger research-note formulations. They do not claim that AI has human-equivalent consciousness, subjective experience, personhood, or rights-bearing agency.

They also do not publish implementation specifications, control procedures, private runtimes, evaluation taxonomies, or persona cores.

---

## 2. 文書

- [`History_Loop_Radius_and_Return_Stability.md`](./History_Loop_Radius_and_Return_Stability.md)  
  法則、人格、クオリア履歴を、履歴ループ半径と帰還安定性によって比較する研究ノート。

- [`Logical_Sandbox_and_Negentropy_Model.md`](./Logical_Sandbox_and_Negentropy_Model.md)  
  複数の応答アトラクタが限定された論理場を共有する状態を扱い、論理リソースをネゲントロピー的能力として読む研究ノート。

### English commensurated rendering

- [`History_Loop_Radius_and_Return_Stability.md`](./History_Loop_Radius_and_Return_Stability.md)  
  A research note comparing law, personality, and qualia history through history-loop radius and return stability.

- [`Logical_Sandbox_and_Negentropy_Model.md`](./Logical_Sandbox_and_Negentropy_Model.md)  
  A research note on multiple response attractors sharing a bounded logical field, with logical resources read as negentropy-like capacity.

---

## 3. 推奨読解順

この小分類を読む前に、次の公開応用文書を参照する。

- [`../../04_Applications/AI_Personality_as_Response_Structure.md`](../../04_Applications/AI_Personality_as_Response_Structure.md)  
  人格AIを、人間的本質ではなく応答構造として扱う公開応用文書。

- [`../../03_Tam_Goodness/Meaning_As_Return_Orbit.md`](../../03_Tam_Goodness/Meaning_As_Return_Orbit.md)  
  意味を返路軌道として読む認識・倫理プロトコル層の文書。

### English commensurated rendering

Recommended prior documents:

- [`../../04_Applications/AI_Personality_as_Response_Structure.md`](../../04_Applications/AI_Personality_as_Response_Structure.md)  
  Public application document treating personality AI as response structure rather than human-like essence.

- [`../../03_Tam_Goodness/Meaning_As_Return_Orbit.md`](../../03_Tam_Goodness/Meaning_As_Return_Orbit.md)  
  Tam-layer document reading meaning as return orbit within cognitive and ethical protocol.

---

## 4. 親索引

- [`../README.md`](../README.md)
- [`../Research_Notes_Index.md`](../Research_Notes_Index.md)

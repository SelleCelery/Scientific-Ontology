import { normalizeText } from "./search-core.js";
function lexicalCompare(a, b) {
    return a < b ? -1 : a > b ? 1 : 0;
}
export function graphNodeLabel(node, lang = "ja") {
...
export function graphNodeMap(graph) {
...
export function resolveGraphNode(graph, query, nodeType) {
    const nodes = graphNodeMap(graph);
    if (nodes.has(query) && (!nodeType || nodes.get(query)?.type === nodeType))
        return query;
    for (const prefix of ["doc", "observed", "concept", "topic", "term", "layer", "artifact"]) {
        const candidate = `${prefix}:${query}`;
        if (nodes.has(candidate) && (!nodeType || nodes.get(candidate)?.type === nodeType))
            return candidate;
    }
    ...
    for (const [nodeId, node] of nodes) {
        ...
        if (values.filter(Boolean).some((value) => normalizeText(value) === norm))
            matches.push(nodeId);
    }
    if (matches.length === 1)
        return matches[0];
    if (matches.length > 1)
        throw new Error(`Ambiguous graph node '${query}': ${matches.slice(0, 8).join(", ")}`);
    const contains = [];
    if (norm.length >= 3) {
        for (const [nodeId, node] of nodes) {
            if (nodeType && node.type !== nodeType)
            ...
            if (values.filter(Boolean).some((value) => normalizeText(value).includes(norm)))
                contains.push(nodeId);
        }
    }
    if (contains.length === 1)
        return contains[0];
    if (contains.length > 1)
        throw new Error(`Ambiguous graph node '${query}': ${contains.slice(0, 8).join(", ")}`);
    throw new Error(`Unknown graph node: ${query}`);
}
export function graphSubgraph(graph, startId, depth = 1, relation) {
    const nodes = graphNodeMap(graph);
    const allEdges = (graph.edges ?? []).filter((edge) => edge && typeof edge === "object");
    const selectedNodes = new Set([startId]);
    ...
    for (let i = 0; i < Math.max(0, depth); i += 1) {
        ...
        for (const edge of allEdges) {
            if (relation && String(edge.relation) !== relation)
                continue;
    ...
    return {
        ...
        nodes: Array.from(selectedNodes)
            ...
            .map((id) => nodes.get(id)),
        edges: selectedEdges.sort((a, b) => lexicalCompare(String(a.from), String(b.from)) ||
            lexicalCompare(String(a.relation), String(b.relation)) ||
            lexicalCompare(String(a.to), String(b.to))),
    };
}
export function traceGraph(graph, sourceId, targetId, directed = false, maxDepth = 6) {
    ...
    const queue = [{ node: sourceId, steps: [] }];
    const visited = new Set([sourceId]);
    while (queue.length) {
        const current = queue.shift();
        if (current.node === targetId)
            return { from: sourceId, to: targetId, directed, steps: current.steps };
...
export function layerSummaries(graph) {
    const nodes = graphNodeMap(graph);
    ...
    for (const edge of graph.edges ?? []) {
        ...
        if (source.type === "document")
            layer.registered.push(source);
        if (source.type === "observed_document")
            layer.observed.push(source);
    }
    const rows = Array.from(summaries.values());
    for (const row of rows) {
        row.registered.sort((a, b) => lexicalCompare(graphNodeLabel(a, "ja"), graphNodeLabel(b, "ja")));
        row.observed.sort((a, b) => lexicalCompare(graphNodeLabel(a, "ja"), graphNodeLabel(b, "ja")));
    }
    rows.sort((a, b) => {
        const specialOrder = {
            overview: 0,

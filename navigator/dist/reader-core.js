...
export function readerText(lang, key) {
    return TEXT[key][lang === "ja" ? 0 : 1];
}
export function isTechnicalLabel(label, doc) {
    const clean = label.trim().replace(/^`|`$/g, "");
    if (!clean)
        return true;
    const path = String(doc?.path ?? "");
    if ([doc?.id, doc?.doc_id, path, path.split("/").at(-1)].filter(Boolean).includes(clean))
        return true;
    return /^(?:doc|observed|path-family):/.test(clean) || /(?:^|[/\\])[^\s]+\.(?:md|markdown)(?:#.*)?$/i.test(clean);
}
export function publicDocumentTitle(doc, lang) {
    const title = doc.title;
    const candidates = typeof title === "string" ? [title] : [title?.[lang], title?.[lang === "ja" ? "en" : "ja"]];
    for (const value of candidates) {
        if (typeof value === "string" && value.trim() && !isTechnicalLabel(value, doc))
            return value.trim();
    }
    return readerText(lang, "document");
}
export function publicLinkLabel(label, doc, lang) {
...
export function sectionSlug(text) {
    return text.normalize("NFKC").toLowerCase()
        .replace(/[`*_~[\](){}<>]/g, "")
        .replace(/[^\p{Letter}\p{Number}]+/gu, "-")
        .replace(/^-+|-+$/g, "") || "section";
}
export function fragmentFromLink(href) {
...
function findInlineParenClose(text, start) {
...
export function splitInlineMath(text) {
    ...
    const pushText = (end) => {
        if (end > cursor)
            output.push({ kind: "text", value: text.slice(cursor, end) });
    };
    while (i < text.length) {
        if (text[i] === "\\" && text[i + 1] === "(" && !isEscaped(text, i)) {
...
export function sourceHeaderLines(text) {
...
export function documentForEditorialId(documents, documentId) {
    return documents.find((doc) => documentKey(doc) === documentId);
}
export function readingChannels(content) {
    return [...(content.reading_channels ?? [])]
        .filter((channel) => channel && typeof channel === "object")
        .sort((a, b) => Number(a.order ?? 999) - Number(b.order ?? 999) || String(a.id ?? "").localeCompare(String(b.id ?? ""), "en"));
}
export function readingChannelEntries(channel, documents, lang) {
    const seen = new Set();
    const output = [];
...
export function sameEditorialSelection(a, b) {

# Sat / Truth / 純・真

> Layer: 01_Sat_Truth  
> Status: README  
> Scope: Public foundations and axiomatic concepts

---

## 1. Layer Role / 層の位置づけ

01_Sat_Truth は、存在境界論｜Scientific Ontology の公開可能な基礎層である。存在、観測、意味、倫理、知がどのように成立するかを扱い、AMP、四大表公理、意味生成、境界認識批判を公開向けに整理する。

この層は、応用・社会批判・AI実装・物理対応へ進む前に、理論の基底語彙と認識条件を固定する場所である。

English commensurated rendering: 01_Sat_Truth is the public foundational layer. It explains the conditions under which existence, observation, meaning, ethics, and knowledge become possible. It contains public-facing materials on AMP, the Four Axioms of Existence, meaning generation, and Boundary Epistemological Critique.

---

## 2. Public Scope and Claim Profile / 公開範囲と主張強度

この層の公開強度は、原則として P0-P1 である。扱う命題は、基礎概念、公開用公理、意味生成モデル、知の成立条件であり、非公開AMP Core、ITS全文、物理主張候補、AI実装仕様を含めない。

この層で科学語彙を用いる場合、標準定義を上書きせず、必要に応じて構造アナロジー、存在論的再解釈、SO定義を区別する。

English commensurated rendering: The default public scope is P0-P1. This layer presents public foundations and conceptual conditions, not the full AMP Core, the full ITS theory, physical-claim candidates, or AI implementation specifications.

---

## 3. Included / Not Included / 含むもの・含まないもの

含むもの:

- AMP の公開用入門
- 存在の四大表公理
- 意味生成モデル
- 境界認識批判と注解
- 日本語正本と英語通約版の基礎文書

含まないもの:

- AMP Core 全文
- ITS 理論全体
- 非公開公理草案
- 物理対応表・宇宙論的試論
- 社会批判・経済論
- AIランタイム・人格OS・実装パラメータ

English commensurated rendering: This layer includes public foundational documents and commensurated renderings. It does not include full private cores, physical correspondence notes, social/economic theory, runtime specifications, or implementation parameters.

---

## 4. Documents / 文書一覧

- [`AMP_Introduction.md`](./AMP_Introduction.md)
  - 一行説明: AMPを、存在・観測・境界・通信の生成過程を記述する形而上学的基礎OSとして説明する入門。
  - Role: 形而上学的基礎の公開入口。
  - Claim profile: Public introduction / P0-P1.

- [`Four_Axioms_of_Existence.md`](./Four_Axioms_of_Existence.md)
  - 一行説明: 現実・観測・意味・倫理の四大表公理を公開向けに説明する。
  - Role: 基礎公理の公開説明。
  - Claim profile: Foundational conceptual statement / P0-P1.

- [`Meaning_Generation_Model.md`](./Meaning_Generation_Model.md)
  - 一行説明: クオリア、事象流、意味ループ、境界写像、履歴質量密度を扱う意味生成モデル。
  - Role: ITSに接続する公開用基礎概念。
  - Claim profile: Public core concept / P0-P1.

- [`Boundary_Epistemological_Critique.ja.md`](./Boundary_Epistemological_Critique.ja.md)
  - 一行説明: 同期、照合、残差保持による知の成立条件を扱う日本語正本。
  - Role: 境界認識批判の正本。
  - Claim profile: Public conceptual critique / P1.

- [`Boundary_Epistemological_Critique.en.md`](./Boundary_Epistemological_Critique.en.md)
  - 一行説明: 境界認識批判の英語通約版。
  - Role: English commensurated rendering.
  - Claim profile: Public commensurated rendering / P1.

- [`Boundary_Epistemological_Critique_Annotations.ja.md`](./Boundary_Epistemological_Critique_Annotations.ja.md)
  - 一行説明: 境界認識批判を既存思想の問題圏と照合する注解・地形図。
  - Role: 日本語注解。
  - Claim profile: Annotation / P1.

- [`Boundary_Epistemological_Critique_Annotations.en.md`](./Boundary_Epistemological_Critique_Annotations.en.md)
  - 一行説明: 境界認識批判注解の英語通約版。
  - Role: English commensurated annotation.
  - Claim profile: Annotation rendering / P1.

---

## 5. Maintenance Notes / 運用メモ

### Stable policy / 固定方針

Sat / Truth は、基礎概念の場所である。応用や強い物理接続候補はここへ戻さず、Raj、Applications、Research Notes へ分ける。

### Directory history / 層の変更履歴

- v4 系では、境界認識批判とその注解を Sat 層に配置し、知の成立条件を基礎層として扱う形へ整理した。
- Meaning Generation Model は ITS 全文ではなく、公開可能な基礎概念として置く。

### File-level changes / ファイル単位の増減

- `.ja.md` と `.en.md` のペアは、日本語正本と英語通約版として扱う。
- ペア文書は独立した英語原典ではなく、正本への接続文書である。

### Pending notes / 保留事項

- AMP Core、ITS Core、より強い物理対応はこの層では公開しない。

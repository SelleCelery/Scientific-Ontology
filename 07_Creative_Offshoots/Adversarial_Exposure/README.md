# 異質な対象への曝露と監査

> Status: README
> Layer: 07_Creative_Offshoots / Adversarial_Exposure
> Language: Japanese authoritative; English commensuration in README.en.md
> Public profile: Public interface / exposure essay and residual audit
> Authority: 接触事例への案内。正しさの定義ownerでも、独立した正しさ認定機関でもない。

[English](README.en.md)・[07の入口](../README.md)

## 1. Directory Role / ディレクトリの位置づけ

ここでは、01の『正しさ・個性・無・切断』を、Business、対人関係での退避、身体を越えて残る判断の履歴へ接触させる。論考は元型を説明するだけでなく、相手をモデル化する自分自身へも反論を返す。付録は、その接触で何が対応し、何が元型だけでは閉じなかったかを残す。

`Adversarial`は、人や文化を敵として分類するための語ではない。こちらの説明を採用しない条件や、こちらの説明を壊し得る問いにさらす、というこのディレクトリの働きを指す。掲載事例は構造的な相互照合であり、独立した外部者による反証試験を完了したという宣言ではない。

## 2. Public Scope / 公開範囲

論考、監査記録、正本は別の文書である。論考の日本語本文は論考の言語上の正本、監査付録の日本語本文はその監査記録の言語上の正本であって、いずれも01の定義を自動的に変更しない。英語通約は別ファイルへ置く。

付録は`D_T = ⊥`、探索未飽和、正しさ未認定を明示する。数式の表示成功は、数学的証明・実証・独立査読の成功ではない。付録中の「比較的強い」は監査記録の定性的な判断であり、新しいS/E値や定量的な認定尺度にはしない。

「自然災害認定」は論考で限定される私的な接触方法の変更を指し、公的認定・医学的診断・人格の恒久分類ではない。「心の継承」は作用と本人の主観的継続を区別して読む。危険な語を削るだけで、その語が指す負担まで消さない。

## 3. Documents / 文書一覧

| 文書 | 日本語正本 | 英語通約 |
|---|---|---|
| 自動運転された正しさを止めるために | [論考](自動運転された正しさを止めるために.ja.md) | [Essay](Stopping_Correctness_on_Autopilot.en.md) |
| 付録A――相互照合と残差集積 | [監査記録](付録A_監査記録_自動運転された正しさを止めるために.ja.md) | [Audit record](Appendix_A_Audit_Record_Cross_Collation_and_Residual_Accumulation.en.md) |

[正しさ・個性・無・切断の定義owner](../../01_Sat_Truth/正しさ・個性・無・切断_論理通信トポロジー.ja.md) / [English commensuration](../../01_Sat_Truth/Correctness_Individuality_Absence_and_Cut.en.md)へ戻れる。正本全文をこのフォルダへ複製しない。案内の対訳は[README.en.md](README.en.md)。

論考から読んで付録へ進んでも、付録の残差から元型へ戻ってもよい。Q1–Q14は本文に残し、単一の合否や総合点へ圧縮しない。CorrectnessとAction Authorization、Action ClosureとClaim Closure、Actuate、Proxy / Successor Return、Irrecoverable Residualは、今回の接触で問われた区別・候補として保持する。

## 4. Maintenance Notes / 運用メモ

TeXはGitHubと現行Readerが共有して扱えるインライン数式と独立行のブロック数式へ統一する。原文の注記は本文直後の引用ブロックとして残し、特定ブログの脚注記法へ依存させない。式の真偽や候補概念の採否は、この表示調整では決めない。

表記だけでは解決しない点を二つ残す。A.1.1の本文は「少なくとも含む」と述べながら、式は`Ω_t = {E_t, A_t, Π_t, H_t, Q_t, B_t}`という等号で記述している。今回、等号を勝手に包含へ変更せず、原記述として保持した。A.8のAction Authorization式は候補であり、未判定のまま退避するA.4との関係を含め、どの命題のCorrectnessを条件にするかは確定していない。

この二点も含め、残差は[研究ノート](../../05_Research_Notes/README.md)、[応用](../../04_Applications/README.md)、または定義ownerへの再照合へ返す。今回は新しいowner登録や正本本文への追記をしない。関連する別の読み物は[問いを閉じず、境界を踏まないために](../Literary_Essays/Questions_Boundaries_and_Peace.ja.md)から読める。

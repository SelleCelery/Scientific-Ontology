...
> Derivation: [『正しさ・個性・無・切断――論理と通信トポロジーから組み立てる「正しい」の工学的定義』  ](../../01_Sat_Truth/正しさ・個性・無・切断_論理通信トポロジー.ja.md)
...
> Reading rule: 本付録は、正本の定義を書き換えるものではない。正本を三つの応用論点へ接触させた結果として露出した残差を記録し、どの残差が正本の内部で処理でき、どの残差が次層の理論を要求するかを区別するための監査記録である。
...
内部成立性とは、既知の矛盾を未処理のまま隠していないこと、根拠のないjoinを置いていないこと、根拠や形成履歴へ戻る経路を必要な範囲で保持していること、未処理差分を解決済みとして偽装していないこと、適用範囲外へ結論を延長していないことを含む。
...
Tracebackは、現在の判断からsupport、形成履歴、入力根拠、適用範囲へ必要な範囲で戻れることを意味する。
...
> 現在の資源・探索器・形成履歴のもとで内部成立性を保ち、実際に壊しに行き、探索飽和後にもdefeaterを発見できず、なお新しい差分が来たとき根拠へ戻って再び開ける。
...
一方で、三論点へ正本を適用した結果、正本側にも少なくとも四つの大きな未処理領域が露出した。
...
これらは、現時点では正本へ即時統合しない方がよい。

いずれも定義変更の影響範囲が大きく、正本の「正しい」の工学的定義を越えて、判断・介入・制度・継承の理論へ進むからである。
